...
class Admin():
    ...
    def create_app(self, owner, title, layout=[], script = [], sketch = [], icon = ''):
        ...
        if title not in user['apps']:
            user['apps'].append(title)
            self.data.apps.insert_one(app)
            self.data.users.update({'email': owner}, user)
        else:
            raise Exception(f'App with name {title} already exists')
        return app

    def open_app(self, email, appname):
        app = self.data.apps.find_one({'title': appname, 'owner': email}, {'_id':0})
        if not app:
            raise Exception('App not found')
        user = self.data.users.find_one({'email': email}, {'apps':1})
        if [dapp for dapp in user['apps'] if dapp==app['title']]:
            return app
        else:
            raise Exception('User does not have access to app')

    def save_app(self, owner, app):
        user = self.data.users.find_one({'email': owner})
        if app['title'] in user['apps']:
            app['owner'] = owner
    ...
    def get_apps(self, owner):
        ...
        for lo in user['apps']:
            try:
                app = self.data.apps.find_one({'_id': lo})
